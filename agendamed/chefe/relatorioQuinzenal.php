...
function Header()
{
    ...
    $this->Cell(80);
    // Title
    $this->Cell(50,10,utf8_decode('Relatório Quinzenal'),1,0,'C');
    // Line break
    $this->Ln(20);
}
...
$pdf = new PDF();
$pdf->SetTitle(utf8_decode('Relatório Quinzenal'));
$pdf->AliasNbPages();
$pdf->AddPage();
...
for($d = 1; $d <= 15; $d++){
	$hoje = date("d/m/Y", mktime(0, 0, 0, date("m"), date("d") - $d,  date("Y")));
...
for($d = 1; $d <= 15; $d++){
	$hojedois = date("d/m/Y", mktime(0, 0, 0, date("m"), date("d") - $d,  date("Y")));

	$selectAllCon = mysqli_query($conn, "SELECT * FROM AgendamentoConsulta WHERE DT_AgendamentoConsulta = '$hojedois' ORDER BY NM_Situacao DESC") or die(mysqli_error($conn));
	$resultAllCon += mysqli_num_rows($selectAllCon);
