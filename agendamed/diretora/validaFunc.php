<?php  
	...
	if ($tipo == 1){
		$queryFunc = mysqli_query($conn, "SELECT NM_Funcionario, NR_Cpf FROM Funcionario WHERE NR_Cpf = '$cpfF'") or die(mysqli_error($conn));
		$mostraFunc = mysqli_fetch_array($queryFunc);
		$resultFunc = mysqli_num_rows($queryFunc);
		if ($resultFunc > 0){
			$_SESSION["cadastroFuncErro"] = $mostraFunc["NM_Funcionario"] . " portador do CPF " . $mostraFunc["NR_Cpf"] . " já está cadastrado.";
			header("Location: cadastroFunc.php");
			die();
		} else {
			$insertFunc = mysqli_query($conn, "INSERT INTO Funcionario VALUES ('$cpfF', '0', '$nome', '$senha', 'Recepcionista', '$sexo', '1', '$ubs')") or die(mysqli_error($conn));
			$tpF = "Recepcionista";
		}
	}
	...
	if ($tipo == 2){
		...
		if ($resultFunc > 0){
			...
			$insertFunc = mysqli_query($conn, "INSERT INTO Funcionario VALUES ('$cpfF', '1', '$nome', '$senha', 'Chefe', '$sexo', '1', '$ubs')") or die(mysqli_error($conn));
			$tpF = "Chefe de UBS";
		}
	}

	if ($tipo == 3){
		$queryFunc = mysqli_query($conn, "SELECT NM_Medico, NR_Cpf, NR_Crm FROM Medico WHERE NR_Cpf = '$cpfF'") or die(mysqli_error($conn));
		$mostraFunc = mysqli_fetch_array($queryFunc);
		$resultFunc = mysqli_num_rows($queryFunc);
		if ($resultFunc > 0){
			$_SESSION["cadastroFuncErro"] = $mostraFunc["NM_Medico"] . " portador do CPF " . $mostraFunc["NR_Cpf"] . " já está cadastrado.";
			header("Location: cadastroFunc.php");
			die();
		} else {
			$insertFunc = mysqli_query($conn, "INSERT INTO Medico VALUES ('$cpfF', '$crm', '$nome', '$especialidade', '$sexo', '1','$ubs')") or die(mysqli_error($conn));
			$tpF = "Médico";
		}
	}

	if ($tipo == 4){
		$queryFunc = mysqli_query($conn, "SELECT NM_Enfermeiro, NR_Cpf, NR_Coren FROM Enfermeiro WHERE NR_Cpf = '$cpfF'") or die(mysqli_error($conn));
		$mostraFunc = mysqli_fetch_array($queryFunc);
		$resultFunc = mysqli_num_rows($queryFunc);
		if ($resultFunc > 0){
			$_SESSION["cadastroFuncErro"] = $mostraFunc["NM_Enfermeiro"] . " portador do CPF " . $mostraFunc["NR_Cpf"] . " já está cadastrado.";
			header("Location: cadastroFunc.php");
			die();
		} else {
			$insertFunc = mysqli_query($conn, "INSERT INTO Enfermeiro VALUES ('$cpfF', '$coren', '$nome', '$sexo', '1','$ubs')") or die(mysqli_error($conn));
			$tpF = "Enfermeiro";
		}
	}
		...
		<div class="row fRow">
			...
			<div class="col-xs-9 col-xs-offset-3 text-justify fMain">
				<div class="col-xs-12 branco sombra cadastroPront">
					<h1 class="text-center">Funcionario cadastrado com sucesso.</h1>	
					<center>
						<div class="row">
							<?php 
								UNSET($_SESSION["cpfF"]);
								UNSET($_SESSION["nome"]);
								UNSET($_SESSION["tipo"]);
								echo "<br><br><br><h4 style='font-weight: 300;''>" . $nome . " portador do CPF " . $cpfF . " foi cadastrado com sucesso como " . $tpF . ".</h4><br>";
							?>
						</div>
						<div class="row">
							<div class="col-xs-12 btnSucesso">
							<a class="btn btn-default" href="cadastroFunc.php" role="button">Cadastrar outro Funcionario</a>
